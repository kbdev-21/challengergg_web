...
export default function FullRuneBoard({rune}) {
  ...
  return (
    <div className={"grid grid-cols-2 w-full"}>
      <div className={"flex flex-col gap-4 xs:gap-6 px-5 border-r border-bg3"}>
        <MainRunes style={rune.mainStyle}/>
        <NineSubRunes style={rune.mainStyle}/>
      </div>
      <div className={"flex flex-col justify-end px-5"}>
        <NineSubRunes style={rune.subStyle}/>
      </div>
    ...
    return (
      <div className={`flex justify-between w-full`}>
        {mainRunes.map(runeId => (
          <div key={runeId} className={`${imgSizeClassName} ${!rune.selections.includes(Number(runeId)) && "grayscale opacity-40"} bg-bg1 rounded-[100%] p-[1px]`}>
      ...
      <div className={"w-full flex flex-col gap-4 xs:gap-6"}>
        <div className={"flex justify-between w-full "}>
          {nineRunes.slice(0,3).map(runeId => (
            <img key={runeId} alt={"rune-img"} src={getRuneImageUrl(runeId)} className={`${imgSizeClassName} ${!rune.selections.includes(Number(runeId)) && "grayscale opacity-40"}`}/>
          ))}
        </div>
        <div className={"flex justify-between w-full "}>
