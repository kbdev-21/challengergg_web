  ...
  return (
    <div className={"pt-10"}>
      ...
      <div className={"flex justify-center px-2"}>
        <div className={"w-full bg-bg2 border border-bg3 rounded-md flex flex-col gap-4 max-w-screen-lg"}>
          {/* Info section */}
          <div className={"flex gap-4 p-6"}>
            ...
            <div className={"flex flex-col gap-3"}>
              <div className={"flex gap-0 xs:gap-2 flex-col xs:flex-row"}>
...
function OverviewSection({playerData}) {
  ...
  const {
    ...
    queryFn: () => fetchMatchesByPuuid(puuid, 0, 15, region)
  })
  ...
  return (
    <div className={"flex justify-center px-2"}>
      <div className={"flex flex-col lg:flex-row gap-2 mt-2 w-full max-w-screen-lg"}>
        {/* Ranks display */}
  ...
  function RankDisplay({index}) {
    return (
      <>
        <div className={"font-[400] text-sm"}>{queueEnumToTextMap[playerData.ranks[index].queue]}</div>
          ...
          <div className={"flex w-full justify-between mt-3 h-[45px]"}>
            <div className={"flex flex-col justify-between"}>
              <div className={"font-[500] text-base"}>
                {upperCaseFirstLowerCaseAll(playerData.ranks[index].tier,)} {!["CHALLENGER", "GRANDMASTER", "MASTER"].includes(playerData.ranks[index].tier) ? playerData.ranks[index].division : ""}
              </div>
              <div className={"text-xs text-text2"}>
                {playerData.ranks[index].points} LP
              </div>
            </div>
